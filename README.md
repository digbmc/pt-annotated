...
- The Dev site is served via GitHub pages here: https://digbmc.github.io/pt-annotated/
- The production site is hosted on a Domain of One's Own account managed and owned by José Vergara and served here: https://ptannotated.com/
- To update the production site, you will have to do a local build and then upload the contents of the _site folder using an FTP account
- Build instructions:
  - Use `jekyll build --config _config_production.yml` 
  - _config_production.yml is a separate config file that includes url and collection settings specific to the production environment.
