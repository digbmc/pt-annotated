...
Edges connect a source node (the starting point) to a target node (the end point), so each collaborator is a source, and have a directed arrow leading to their targets, which in this instance is the films they worked on. Functionally, this meant a spreadsheet with a column for the sources and a column for the target, if they are in the same row they are connected. A source can be connected to multiple targets.  
